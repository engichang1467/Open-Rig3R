...
class Wayve101Dataset(Dataset):
        ...
        for cam in self.camera_dirs:
            ...
            for f in selected_files:
                ...
                if self.transforms:
                    ...
                    # img = torch.tensor(img.resize(self.image_size)).permute(2,0,1).float()/255.0
    ...
    def _load_metadata(self, seq_path):
        ...
        # cam2rig = torch.eye(3).unsqueeze(0).repeat(len(self.camera_dirs) * self.n_frames, 1,1)
        ...
        # Flatten each 3x3 matrix into a 9-dimensional vector for the decoder
        # cam2rig = cam2rig.view(num_tokens, 9)  # shape (num_tokens, 9), batch dim added
        ...
        def read_bin_point3D(file_path):
            ...
            with open(file_path, 'rb') as f:
                ...
                for _ in range(num_points):
                    ...
                    # rgb = data[32:35]  # can ignore
                    # error = struct.unpack('<d', data[35:43])[0]
...
#         # Add batch dimension for consistency: (B, M, 9)
#         # Dataset returns single sequence per item, so batch dim = 1
#         cam2rig = cam2rig.unsqueeze(0)  # (1, num_tokens, 9)
...
#         # Ensure shape (N,3)
#         if pointcloud.ndim == 1 and pointcloud.shape[0] == 3:
