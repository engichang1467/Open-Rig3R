...
# # --------------------
# # Number of subfolders to download in parallel.
# MAX_PARALLEL=4 # Use 8 if there's 32+ GB of RAM

# # Define the list of subfolders
# subfolders="camera_box camera_calibration camera_hkp camera_image camera_segmentation camera_to_lidar_box_association lidar lidar_box lidar_calibration lidar_camera_projection lidar_camera_synced_box lidar_hkp lidar_pose lidar_segmentation projected_lidar_box stats vehicle_pose"

# # Define the file IDs
# file_ids="10017090168044687777_6380_000_6400_000 10023947602400723454_1120_000_1140_000 1005081002024129653_5313_150_5333_150 10061305430875486848_1080_000_1100_000 10072140764565668044_4060_000_4080_000"

# echo "Starting memory-optimized parallel downloads (Batch size: $MAX_PARALLEL)..."
...
#   (
#     # Create directory
#     mkdir -p "data/waymo_mini/train/$subfolder/"
    
#     # Build path list
#     gcs_paths=""
#     for file_id in $file_ids; do
#       gcs_paths="$gcs_paths gs://waymo_open_dataset_v_2_0_1/training/$subfolder/${file_id}.parquet"
#     done
    
#     # Run gsutil (output silenced)
#     gsutil -m cp $gcs_paths "data/waymo_mini/train/$subfolder/" > /dev/null 2>&1
    
#     echo "Completed: $subfolder"
#   ) &

#   # Standard POSIX math to increment counter
#   counter=$((counter + 1))

#   # Standard POSIX modulo check
#   # If counter is divisible by MAX_PARALLEL, wait for the batch to finish
#   if [ $((counter % MAX_PARALLEL)) -eq 0 ]; then
#     echo "--- Batch limit ($MAX_PARALLEL) reached. Waiting for current downloads... ---"
#     wait
#   fi
